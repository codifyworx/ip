...
- Workflows should target stable runner-class labels such as `codifyworx-ci` for private generic request jobs, repo-scoped labels such as `codify-ip-ci` for public trusted-host jobs, or `codifyworx-android` for Android jobs; do not bake hostnames such as `nuc01` into app repo workflow labels.
- Do not put the generic `codifyworx-ci` label on a capability runner unless that runner can satisfy every generic job. GitHub schedules a job on any runner containing all requested labels.
...
- Before pushing or rerunning CI after runner infrastructure changes, validate rendered runner labels and any request bridge permissions locally. CI should confirm a tested path, not discover basic runner wiring mistakes.
